...
import { extractTokensFromEvents, TokensFromEvents, supportsInterfaces } from "./shared/utils";
...
type TransferFn = (from: string, to: string, tokenId: BigNumberish, sender: SignerWithAddress) => Promise<TransactionResponse>;

const transferFrom = function(this: Mocha.Context, from: string, to: string, tokenId: BigNumberish, sender: SignerWithAddress) {
	return this.token.connect(sender).transferFrom(from, to, tokenId);
}

const safeTransferFromWithoutData = function(this: Mocha.Context, from: string, to:string, tokenId: BigNumberish, sender: SignerWithAddress) {
	return this.token.connect(sender)["safeTransferFrom(address,address,uint256)"](from, to, tokenId);
}

const safeTransferFromWithData = function(this: Mocha.Context, from: string, to:string, tokenId: BigNumberish, sender: SignerWithAddress) {
...
export function shouldBehaveLikeERC721(): void {
    ...
    describe("ownerOf()", function () {
      ...
      it("throws for queries to tokens assigned to the zero address", async function () {
        await expect(this.token.ownerOf(TOKEN_ID_NON_EXISTENT)).to.be.revertedWith(
    ...
    context("transfer functions", function () {
			...
			const expectedTransferBehavior = function(transferFunc: TransferFn, tokenId: BigNumberish) {
				...
				var tx: Promise<TransactionResponse>;
				var fromBalance: BigNumber;
				var toBalance: BigNumber;

				context("when transferring owner's token to to a non-zero address", function () {
					beforeEach(async function () {
					...
					it("adjust's the owner's balance", async function () {
						expect(await this.token.balanceOf(this.from.address)).to.equal(
							this.from != this.to ? fromBalance.sub(BigNumber.from("1")) : fromBalance
						);
					});

					it("adjust's the receiver's balance", async function () {
						expect(await this.token.balanceOf(this.to.address)).to.equal(
							this.from != this.to ? toBalance.add(BigNumber.from("1")) : toBalance
						);
					});
					...
					it('emits a Transfer event', async function () {
						expect(tx).to.emit(this.token, 'Transfer').withArgs(this.from.address, this.to.address, tokenId);
					});

					it('emits an Approval event', async function () {
						expect(tx).to.emit(this.token, 'Approval').withArgs(this.from.address, constants.AddressZero, tokenId);
					});
				...
				it('throws when `_from` is not the actual owner', async function () {
					await expect(
						transferFunc.bind(this)(this.receiver.address, this.to.address, tokenId, this.sender)).to.be.revertedWith("ERC721: transfer of token that is not own");
				});

				it('throws when `_tokenId` is not valid', async function () {
					await expect(
						transferFunc.bind(this)(this.from.address, this.to.address, BigNumber.from("9999"), this.sender)).to.be.revertedWith("ERC721: operator query for nonexistent token");
				});

			}

			const expectedTransferFunctionBehavior = function(transferFunc: TransferFn) {
				...
				context('when authorized operator invokes transfer to receiver', function () {
					beforeEach(async function () {
				...
				context('when approved address invokes transfer to receiver', function () {
					beforeEach(async function () {
			...
			const expectedSafeTransferFunctionBehavior = function(transferFunc: TransferFn) {
				...
				context('when transferring to a contract', function () {
					...
					describe('when the contract is a valid ERC721 receiver', function () {
						...
						beforeEach(async function () {
							this.to = await (new MockERC721Receiver__factory(this.deployer)).deploy(ERC721_RECEIVER_MAGIC_VALUE, false);
						});

						 expectedTransferFunctionBehavior(transferFunc);

						it('calls onERC721Received()', async function () {
							await expect(transferFunc.bind(this)(this.from.address, this.to.address, TOKEN_ID_0, this.sender)).to.emit(this.to, 'ERC721Received').withArgs(this.from.address, this.sender.address, TOKEN_ID_0, transferFunc == safeTransferFromWithData ? DATA : "0x");
						});

					});

					it('throws when onERC721Received() returns the wrong magic value', async function () {
							this.to = await (new MockERC721Receiver__factory(this.deployer)).deploy("0xDEADBEEF", false);
							await expect(
								transferFunc.bind(this)(this.from.address, this.to.address, TOKEN_ID_0, this.sender)).to.be.revertedWith("ERC721: transfer to non ERC721Receiver implementer");
					});

					it('throws when onERC721Received() throws', async function () {
							this.to = await (new MockERC721Receiver__factory(this.deployer)).deploy("0xDEADBEEF", true);
							await expect(
								transferFunc.bind(this)(this.from.address, this.to.address, TOKEN_ID_0, this.sender)).to.be.revertedWith("MockERC721Receiver: throwing");
					});
		...
		describe("approve()", function () {
			...
			const successfulApprovalBehavior = function() {
				...
				it('correctly clears or sets the new approved address', async function () {
					expect(await this.token.getApproved(TOKEN_ID_0)).to.equal(this.clearedOrApprovedAddress);
				});

				it('emits an approval event', async function () {
					expect(tx).to.emit(this.token, 'Approval').withArgs(this.from.address, this.clearedOrApprovedAddress, TOKEN_ID_0);
				});
			...
			context('when operator approves another address', function () {
				beforeEach(async function () {
						this.clearedOrApprovedAddress = this.approved.address;
						await this.token.setApprovalForAll(this.operator.address, true);
						tx = this.token.connect(this.operator).approve(this.clearedOrApprovedAddress, TOKEN_ID_0);
						await Promise.resolve(tx);
				});
				successfulApprovalBehavior();
			});
			...
			it('throws when approver is not the owner or an approved operator', async function () {
					await expect(
						this.token.connect(this.approved).approve(this.approved.address, TOKEN_ID_0)
		...
		describe("setApprovalForAll()", function () {
			...
			const setApprovalForAllBehavior = function(approved: boolean) {
				...
				it("correctly changes the operator's authorization", async function () {
					expect(await this.token.isApprovedForAll(this.from.address, this.operator.address)).to.equal(approved);
				});

				it('emits an ApprovalForAll event', async function () {
					expect(tx).to.emit(this.token, 'ApprovalForAll').withArgs(this.from.address, this.operator.address, approved);
				});
		...
		describe("getApproved()", function () {
			...
			it('should return the approved address for a token', async function () {
				expect(await this.token.getApproved(TOKEN_ID_0)).to.equal(this.approved.address);
			});

			it('should return the zero address for tokens with no approved addresses', async function () {
				expect(await this.token.getApproved(TOKEN_ID_1)).to.equal(constants.AddressZero);
			});

			it('throws when `_tokenId` is not valid', async function () {
				await expect(this.token.getApproved(TOKEN_ID_NON_EXISTENT)).to.be.revertedWith("ERC721: approved query for nonexistent token");
			});
		});
		...
		describe("isApprovedForAll()", function () {
			...
			it('should return true for approved operators of an owner', async function () {
				expect(await this.token.isApprovedForAll(this.from.address, this.operator.address)).to.equal(true);
			});

			it('should return false for unapproved operators of an owner', async function () {
				expect(await this.token.isApprovedForAll(this.from.address, this.approved.address)).to.equal(false);
			});
...
export function shouldBehaveLikeERC721Enumerable(): void {
	describe('ERC721Enumerable functionality', function () {
		...
		var circulatingTokens: Number[];

		beforeEach(async function () {
			let events: Event[] = [];
			events.push.apply(events, ((await (await this.token.mint()).wait()).events));
			events.push.apply(events, ((await (await this.token.mint()).wait()).events));
			events.push.apply(events, ((await (await this.token.mint()).wait()).events));
			events.push.apply(events, ((await (await this.token.burn(TOKEN_ID_0)).wait()).events));
			await this.token.transferFrom(this.from.address, this.to.address, TOKEN_ID_1);
		...
		describe('totalSupply()', function () {
			it('returns the correct total supply', async function () {
				expect(await this.token.totalSupply()).to.equal(circulatingTokens.length);
			});

			it('decreases when tokens are burned', async function () {
				await this.token.burn(2);
				expect(await this.token.totalSupply()).to.equal(circulatingTokens.length - 1);
			});
		...
		describe('tokenByIndex()', function () {
			it('enumerates all circulating tokens', async function () {
				const allTokens = await Promise.all(
					[...Array(circulatingTokens.length).keys()].map(i => this.token.tokenByIndex(i)),
				);
				expect(allTokens.map(i => i.toNumber())).to.have.same.members(circulatingTokens);
			});
		...
		describe('tokenOfOwnerByIndex()', function () {
			...
			it('enumerates all owned tokens for an owner', async function () {
				...
				const enumerated = await Promise.all(
					[...Array(numOwned).keys()].map(i => this.token.tokenOfOwnerByIndex(this.from.address, i)),
				)
				...
				expect(enumerated.map(i => i.toNumber())).to.have.same.members(ownedTokens);
			});
