...
import { stopMining, startMining, setNextBlockTimestamp, impersonate, stopImpersonating, encodeParameters, mineBlock, mintN, advanceBlocks, getChainId } from "../shared/utils";
...
export function testRaritySocietyDAOImplLifecycle(): void {
	...
	describe("RaritySocietyDAOImpl lifecycle functionality", function () {
		...
		beforeEach(async function () {
			...
			calldatas = [encodeParameters(['uint256'], [Constants.TIMELOCK_DELAY + 1])];
			await stopMining();
		...
		it('reverts when querying invalid proposals', async function () {
			await expect(this.daoImpl.state(10)).to.be.revertedWith("Invalid proposal ID");
		});
		...
		describe('state: pending', function () {
			...
			it('does not allow queuing a pending proposal', async function () {
				await expect(this.daoImpl.queue(1)).to.be.revertedWith('proposal queueable only if succeeded')
			});
		});
		...
		describe('state: queued', function () {
			it('enables successful proposals to be queued', async function () {
					...
					await mineBlock();
					const eta = (await ethers.provider.getBlock("latest")).timestamp + Constants.TIMELOCK_DELAY;
					expect((await this.daoImpl.proposals(1)).eta).to.equal(eta);
					expect(tx).to.emit(this.daoImpl, Constants.EVENT_PROPOSAL_QUEUED).withArgs(1, eta);
			});

			it('reverts on queuing identical actions', async function () {
					await this.daoImpl.connect(this.voter).propose(targets.concat(targets), values.concat(values), signatures.concat(signatures), calldatas.concat(calldatas), "test repeat");
					await mineBlock();
					await advanceBlocks(Constants.VOTING_DELAY);
					await this.daoImpl.castVote(2, 1);
					await advanceBlocks(Constants.VOTING_PERIOD);
					await mineBlock();
					await expect(this.daoImpl.queue(2)).to.be.revertedWith('identical proposal already queued at eta');
			});

			it('reverts on queueing identical proposals in the same block', async function () {
					await this.token.transferFrom(this.deployer.address, this.admin.address, TOTAL_SUPPLY - 1);
					await mineBlock();
					await this.daoImpl.connect(this.voter).propose(targets, values, signatures, calldatas, "test");
					await this.daoImpl.connect(this.admin).propose(targets, values, signatures, calldatas, "test");
					await mineBlock();
					await advanceBlocks(Constants.VOTING_DELAY);
					await this.daoImpl.castVote(2, 1);
					...
					await expect(this.daoImpl.queue(3)).to.be.revertedWith('identical proposal already queued at eta');
					await mineBlock();
		...
		describe('state: executed', function () {
			it('executes queued proposals', async function () {
					...
					expect(tx).to.emit(this.daoImpl, Constants.EVENT_PROPOSAL_EXECUTED).withArgs(1);

					expect(await this.timelock.delay()).to.equal(Constants.TIMELOCK_DELAY + 1);
			});
		...
		describe('state: canceled', function () {
			...
			it('allows cancellation by anyone if proposer votes drops below threshold', async function () {
				let proposerVotes = await this.token.getCurrentVotes(this.deployer.address);
				const proposalThreshold = await this.daoImpl.proposalThreshold();
				await expect(
					this.daoImpl.connect(this.voter).cancel(1)
				).to.be.revertedWith("only proposer can cancel unless their votes drop below proposal threshold");
			...
			it('emits a ProposalCanceled event', async function () {
				...
				expect(tx).to.emit(this.daoImpl, Constants.EVENT_PROPOSAL_CANCELED).withArgs(1);
			});
			...
			it('does not allow canceling an executed proposal', async function () {
					...
					await expect(this.daoImpl.cancel(1)).to.be.revertedWith('proposal already executed');
			});
		});
		...
		describe('state: defeated', function () {
			...
			it('ensures proposals whose for votes do not pass quorum are defeated', async function () {
					await this.token.transferFrom(this.deployer.address, this.admin.address, TOTAL_SUPPLY - 1);
					await this.daoImpl.connect(this.voter).propose(targets, values, signatures, calldatas, "test2");
					await mineBlock();
					await advanceBlocks(Constants.VOTING_DELAY);
		...
		describe('state: expired', function () {
			it('reverts execution of expired proposals', async function () {
					...
					await mineBlock();
					const ts = (await ethers.provider.getBlock("latest")).timestamp;
					await setNextBlockTimestamp(ts + Constants.TIMELOCK_DELAY + Constants.TIMELOCK_GRACE_PERIOD);
					await mineBlock();
					await expect(await this.daoImpl.state(1)).to.be.equal(STATES.EXPIRED);
					await expect(this.daoImpl.execute(1)).to.be.revertedWith('proposal can only be executed if queued');
		...
		describe('state: vetoed', function () {
			...
			it('throws when trying to veto an executed proposal', async function () {
					...
					await setNextBlockTimestamp(ts + Constants.TIMELOCK_DELAY);
					await this.daoImpl.execute(1);
					await expect(this.daoImpl.connect(this.vetoer).veto(1)).to.be.revertedWith('cannot veto executed proposal');
			});

			it('emits a ProposalVetoed event', async function () {
				const tx = await this.daoImpl.connect(this.vetoer).veto(1);
				expect(tx).to.emit(this.daoImpl, Constants.EVENT_PROPOSAL_VETOED).withArgs(1);
			});
		});
