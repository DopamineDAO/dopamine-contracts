  ...
  describe("RaritySocietyDAOImpl propose functionality", function () {
    ...
    describe("basic proposal functionality", function () {
      ...
      it("reverts when proposer voting power below proposal threshold", async function () {
        ...
        const maxProposalThreshold = (
          await this.daoImpl.maxProposalThreshold()
        ).toNumber();
        expect(maxProposalThreshold).to.equal(
          Math.floor(
            (TOTAL_SUPPLY * Constants.MAX_PROPOSAL_THRESHOLD_BPS) / 10000
          )
        );
        await this.daoImpl
          .connect(this.admin)
          .setProposalThreshold(maxProposalThreshold);
      ...
      it("reverts when too many actions are provided", async function () {
        const actions: string[] = new Array(
          Constants.PROPOSAL_MAX_OPERATIONS + 1
        ).fill(this.token.address);
        await expect(
      ...
      it("generates a proposal with the expected attributes", async function () {
        const tx = await this.daoImpl.propose(
          targets,
          values,
          signatures,
          calldatas,
        ...
        const quorumVotes = Math.max(
          1,
          Math.floor((TOTAL_SUPPLY * Constants.QUORUM_VOTES_BPS) / 10000)
        );
        await expect(tx)
          .to.emit(this.daoImpl, Constants.EVENT_PROPOSAL_CREATED)
          .withArgs(
            1,
            this.deployer.address,
            targets,
        ...
        expect(
          await this.daoImpl.latestProposalIds(proposal.proposer)
        ).to.equal(1);
    ...
    describe("when submitting multiple proposals", function () {
      ...
      it("reverts when an address proposes while having a pending / active proposal", async function () {
        ...
        await expect(
          ...
          "One proposal per proposer - pending proposal already found"
        );
        await mineBlock();
        ...
        await expect(
          ...
          "One proposal per proposer - active proposal already found"
        );
        await mineBlock();
