...
type VoteFn = (voter: SignerWithAddress, proposalId: number, support: number) => Promise<TransactionResponse>;
...
export function testRaritySocietyDAOImplCastVote(): void {
	...
	const castVote = async function(this: Mocha.Context, voter: SignerWithAddress, proposalId: number, support: number) {
		return await this.daoImpl.connect(voter).castVote(proposalId, support);
	}

	const castVoteWithReason = async function(this: Mocha.Context, voter: SignerWithAddress, proposalId: number, support: number) {
		return await this.daoImpl.connect(voter).castVoteWithReason(proposalId, support, VOTE_REASON);
	}

	const castVoteBySig = async function(this: Mocha.Context, voter: SignerWithAddress, proposalId: number, support: number) {
			...
			return await this.daoImpl.connect(voter).castVoteBySig(proposalId, support, v, r, s);
	}
		...
		const expectedVotingBehavior = function(voteFunc: VoteFn, reason: string) {
			context('when votes are expected to be invalid', function () {
				...
				beforeEach(async function () {
					...
					await	this.daoImpl.propose(targets, values, signatures, calldatas, "")
				});
			...
			context('when votes are expected to be valid', function () {
				...
				var expectedVotingPowerDeployer: number;
				var expectedVotingPowerVoter: number;

				beforeEach(async function () {
					await mintN(this.token, TOTAL_SUPPLY);
					await stopMining();
					await this.token.transferFrom(this.deployer.address, this.voter.address, 0);
					await this.token.transferFrom(this.deployer.address, this.voter.address, 1);
					await	this.daoImpl.propose(targets, values, signatures, calldatas, "")
					await mineBlock();

					expectedVotingPowerDeployer = await this.token.balanceOf(this.deployer.address);
					expectedVotingPowerVoter = await this.token.balanceOf(this.voter.address);

					await this.token.transferFrom(this.deployer.address, this.voter.address, 2);
					await this.token.connect(this.voter).transferFrom(this.voter.address, this.deployer.address, 0);
					await advanceBlocks(Constants.VOTING_DELAY);
					await startMining();
				});
				...
				it('appropriately adjusts the proposals voting weights', async function () {
					...
					let deployerReceipt = await this.daoImpl.getReceipt(1, this.deployer.address);
					let voterReceipt = await this.daoImpl.getReceipt(1, this.voter.address);
					let adminReceipt = await this.daoImpl.getReceipt(1, this.admin.address);
					...
					deployerReceipt = await this.daoImpl.getReceipt(1, this.deployer.address);
				...
				it('tallies based on voting power at block of proposal creation', async function () {
					await voteFunc.bind(this)(this.deployer, 1, 2);
					await voteFunc.bind(this)(this.voter, 1, 1);
					const deployerReceipt = await this.daoImpl.getReceipt(1, this.deployer.address);
					const voterReceipt = await this.daoImpl.getReceipt(1, this.voter.address);
					expect(deployerReceipt.votes).to.equal(expectedVotingPowerDeployer);
					expect(voterReceipt.votes).to.equal(expectedVotingPowerVoter);
				});
				...
				it('emits a VoteCast event with an empty reason string', async function () {
						...
						await expect(tx).to.emit(this.daoImpl, Constants.EVENT_VOTE_CAST).withArgs(
