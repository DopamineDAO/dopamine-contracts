...
export function testRaritySocietyDAOImplInitialize(): void {
	...
	describe("RaritySocietyDAOImpl initialize functionality", function () {
		...
		it('correctly initializes all storage variables', async function () {
			...
			expect(await daoImpl.proposalThreshold()).to.equal(Constants.PROPOSAL_THRESHOLD);
			expect(await daoImpl.quorumVotesBPS()).to.equal(Constants.QUORUM_VOTES_BPS);
		...
		it('emits events for setting governance parameters', async function () {
			...
			const daoImpl = await this.daoImplFactory.attach(utils.getContractAddress(tx));
			await expect(tx).to.emit(daoImpl, Constants.EVENT_VOTING_PERIOD_SET).withArgs(0, Constants.VOTING_PERIOD);
			await expect(tx).to.emit(daoImpl, Constants.EVENT_VOTING_DELAY_SET).withArgs(0, Constants.VOTING_DELAY);
			await expect(tx).to.emit(daoImpl, Constants.EVENT_PROPOSAL_THRESHOLD_SET).withArgs(0, Constants.PROPOSAL_THRESHOLD);
			await expect(tx).to.emit(daoImpl, Constants.EVENT_QUORUM_VOTES_BPS_SET).withArgs(0, Constants.QUORUM_VOTES_BPS);
		});
	});
}
