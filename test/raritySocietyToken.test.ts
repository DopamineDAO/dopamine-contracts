...
import { RaritySocietyToken, IProxyRegistry } from "../typechain";
import { raritySocietyTokenFixture, RaritySocietyTokenFixture } from "./shared/fixtures";
...
describe("RaritySocietyToken", function () {
  ...
  beforeEach('instantiate Rarity Society token fixture', async function () {
		({ token: this.token, registry: this.registry } = await loadFixture(raritySocietyTokenFixture));
