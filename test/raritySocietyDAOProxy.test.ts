...
import { RaritySocietyToken, IProxyRegistry, RaritySocietyDAOImpl__factory, RaritySocietyDAOImpl } from "../typechain";
...
describe("RaritySocietyDAOProxy", function () {
  ...
  beforeEach('instantiate fixtures for RaritySocietyDAO', async function () {
		({ token: this.token, timelock: this.timelock, dao: this.dao, daoProxyImpl: this.daoImpl, daoImpl: daoImpl } = await loadFixture(raritySocietyDAOProxyFixture));
	});
	...
	describe('initialization', function () {
		...
		it('throws when implementation setter is not admin', async function () {
			await expect(this.dao.setImpl(constants.AddressZero)).to.be.revertedWith('setImpl may only be called by admin')
		});

		it('throws when implementation being set is not a contract', async function () {
			await expect(this.dao.connect(this.admin).setImpl(constants.AddressZero)).to.be.revertedWith('implementation is not a contract')
		});
		...
		it('can set new implementations', async function () {
			...
			expect(tx).to.emit(this.dao, EVENT_NEW_IMPL).withArgs(daoImpl.address, newImpl.address);
		});
